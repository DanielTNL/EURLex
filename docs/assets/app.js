...
function renderFeed(){
  ...
  items = items.slice(0, 10); // homepage shows latest 10
  els.feed && (els.feed.innerHTML = items.length ? items.map(card).join('') : '<p>No items match.</p>');
}
...
function renderKeyItems(){
  ...
  els.keyItems && (els.keyItems.innerHTML = arr.length ? arr.map(x=>`<li><a href="${esc(x.url)}" target="_blank">${esc(x.text)}</a> <span class="tag">${esc(x.date)}</span></li>`).join('') : '<li>No key items.</li>');
}

function renderTimeline(){
  const arr = REPORTS.filter(r=>passes(r,true)).sort((a,b)=> new Date(b.date)-new Date(a.date));
  els.timeline && (els.timeline.innerHTML = arr.length ? arr.map(r=>`
...
async function maybeAsk(){
  ...
  if(!els.askAi || !els.askAi.checked || !q){ els.answerSection && (els.answerSection.hidden=true); return; }
  if(!API_BASE){ notice('Ask AI is enabled but API_BASE is not set. Open console and run: localStorage.setItem("API_BASE","https://<your-vercel>.vercel.app")'); els.answerSection && (els.answerSection.hidden=true); return; }
  try{
    ...
    els.answer && (els.answer.innerHTML = `<p>${esc(j.answer || 'No model answer (server missing API key).')}</p>`);
    els.aiResults && (els.aiResults.innerHTML = (j.results||[]).map(card).join(''));
  }catch(e){ console.warn(e); els.answerSection && (els.answerSection.hidden=true); }
}
