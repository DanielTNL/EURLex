...
async function loadData() {
  try {
    ...
    els.clearBtn.onclick = () => {
      selectedTags.clear(); selectedSources.clear(); selectedCats.clear();
      els.q.value = '';
      document.querySelector('input[name="datewin"][value="0"]').checked = true;
      dateWindowDays = 0;
      renderAll();
    };
...
function passes(item, isReport = false) {
  ...
  // Sources (only for posts)
  if (selectedSources.size && !isReport && !selectedSources.has(item.source || 'Other')) return false;

  // Categories: check against union of item.tags and item.categories
  ...
  if (selectedTags.size) {
    for (const t of selectedTags) { if (!(item.tags || []).includes(t)) return false; }
  }
...
function renderKeyItems() {
  ...
  els.keyItems.innerHTML = arr.length
    ? arr.map(x => `<li><a href="${esc(x.url)}" target="_blank">${esc(x.text)}</a> <span class="tag">${esc(x.date)}</span></li>`).join('')
    : '<li>No key items.</li>';
}
...
function renderTimeline() {
  ...
  els.timeline.innerHTML = arr.length ? arr.map(r => `
    ...
    <div class="meta">${(r.tags || []).map(t => `<span class="tag">${esc(t)}</span>`).join('')}</div></div></div>`
...
function renderResources() {
  ...
  if (AUDIO.google_drive) {
    cards.push(`<article class="card"><h3>Google Drive</h3><p><a href="${esc(AUDIO.google_drive)}" target="_blank" rel="noopener">Open the Drive folder</a></p></article>`);
  }
