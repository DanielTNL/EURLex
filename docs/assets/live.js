...
const API_BASE = localStorage.getItem('API_BASE') || ''; // optional (chat)
const $ = id => document.getElementById(id);

let POSTS=[], AUDIO={google_drive:"", items:[]};
let selectedTags=new Set(), selectedSources=new Set(), selectedCats=new Set();
let dateWindowDays=0, page=0;
const PAGE_SIZE=30;
...
function setFiltersForChat(){ window.FeedFilters={selectedTags,selectedSources,selectedCats,dateWindowDays}; }
...
function buildPills(){
  ...
  // Categories (fixed list mapped from tags)
...
async function init(){
  try{
    ...
    POSTS=posts||[]; AUDIO=audio||{google_drive:"",items:[]};
    ...
    $('askAi').onchange = ()=>{}; // chat.js handles its own panel
